...
function fetchWorkshops(){
	  ...
	  $.ajax({
			type:'GET',
			url: '/Snap-L-Earn/fetchUserWorkshop/'+userId,
			contentType: 'application/json',
			dataType: 'json',
			success:function(workshops){
				$.each(workshops.workshopResponses, function(i, data){
					$workshops.append('<article id="first" class="container box style1 right"><img src="data:image/jpeg;base64,'+data.image+'" style="width:350px;height:300px" align="left" /><div class="inner"><header><h2>'+data.name+'<br /></h2></header><br />'+data.desc+'<br />Date of Entry : '+data.doe+'</div></article>');
				});
			}
			});
}
